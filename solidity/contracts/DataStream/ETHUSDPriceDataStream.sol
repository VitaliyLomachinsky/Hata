/**
 *Submitted for verification at Sepolia.Arbiscan.io on 2023-11-21
 */
    ...
    StreamsLookupCompatibleInterface
{
    ...
    IVerifierProxy public verifier =
        IVerifierProxy(address(0x2ff010DEbC1297f19579B4246cad07bd24F2488A)); //Arbitrum Sepolia:

    uint256 public latestETHUSDPrice;
    ...
    function performUpkeep(bytes calldata performData) external {
        ...
        int192 price = verifiedReport.price;
        if (price < 0) {
            revert DataStreamError();
        }
        uint256 convertedPrice = uint256(int256(price));

        latestETHUSDPrice = convertedPrice;

        // Log price from report
        emit PriceUpdated(convertedPrice);
    }
